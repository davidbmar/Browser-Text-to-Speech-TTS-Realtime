...
Architecture
How Streaming Works
Text Splitting - Input text is split into sentences
Parallel Generation - Multiple sentences generate simultaneously (configurable)
Sequential Playback - Audio plays in order as chunks become ready
Memory Management - Blob URLs are cleaned up after playback
...
Performance Tips
Enable Auto Warm-Up - Set autoWarmUp: true to pre-load models
Increase Concurrency - Set maxConcurrent: 3-4 for faster generation
Cache Models - Models are cached in browser IndexedDB after first download
Use Appropriate Voice - "medium" quality voices balance size and quality
Browser Support
...
npm run build
Credits
Piper TTS - VITS neural TTS models
@diffusionstudio/vits-web - WebAssembly bindings
